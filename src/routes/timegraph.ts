...
export const timegraphRoute: FastifyPluginCallback = (fastify, _opts_, done) => {
    ...
    fastify.get<{ Body: { requested_times: number[] }; Params: { exp_uuid: string; output_id: string } }>(
        ...
        async (request) => {
            for (const data of await trace_coordinator.fetchTimegraphTree(
                request.params.exp_uuid,
                request.params.output_id,
                request.body.requested_times,
            )) {
                maintainer_manager.dispatch(`TIMEGRAPH`, data);
            }
